% Questão 7
    ...
    % Projete filtros FIR que satisfaçam as especificações do Prob. 5 
    % usando os métodos 

    % a) das Janelas (função fir1)
    % b) Parks-McClellan (função firpm)

    % Especificações do Trabalho 5:

    % i.   Faixa de passagem: 0 - 0.1*pi
    % ii.  Faixa de rejeição: 0.2*pi - 0.6*pi
    % iii. Atenuação de faixa de rejeição > 40dB
    % iv.  Ripple na faixa de passagem < 0.01
...
% a) das Janelas (função fir1)
        ...
        [n_kaiser,Wn_kaiser] = kaiserord([0 0.2 0.6 0.7],[0 1 0],dev);
